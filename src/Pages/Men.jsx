  ...
  useEffect(() => {
    async function fetchData() {
      const res = await axios.get(
        `${process.env.REACT_APP_MEN}?q=${query}&_limit=15&_page=${page}`
      );
      setMen(res.data);
      return res.data;
    }
  ...
  return (
    <main className="my-[78px]    justify-center">
      ...
      <img
        ...
        src="../assets/covers/men.svg"
        alt="men-cover"
      />

      <section className="   flex w-11/12 mx-auto">
        <aside className="w-1/4 bg-orange-500 ">f</aside>
        <div className="w-3/4">
          <Items type={men} name={name} setPage={setPage} />
        </div>
      </section>
    </main>
  );
